...
return declare("dojox.form.uploader.FileList", [formUploaderBase], {
	...
	_progress: function(/*Object*/ customEvent){
		...
		if(customEvent.percent == '100%'){
			this.c.innerHTML = '<img src="app/post/tmpUpload/'+this.pic+'" style="width:70px;height:70px"><img>';
		}
	},
	...
	_onUploaderChange: function(fileArray){
		...
		array.forEach(fileArray, function(f, i){
			this._addRow(i+1, this.getFileType(f.name), f.name, f.size, f.name);
		}, this)
	},

	_addRow: function(index, type, name, size, pic){
