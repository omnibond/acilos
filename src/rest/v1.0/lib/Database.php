...
class Database{
	...
	public function checkRebootSetting(){
		...
		try{
			...
			if(isset($settingsObj['rebootOptions'])){
				return json_encode(array("Response" => $settingsObj['rebootOptions']));
			}
		}catch (Exception $e){
	...
	public function saveRebootSettings(){
		$var = file_get_contents("php://input");
		$obj = json_decode($var, true);
		...
		try{
			...
			return json_encode(array("success" => "Your settings were saved successfully"));
		}catch(Exception $e){
			return json_encode(array("failure" => "there was an error saving your settings"));
		}			
	}
	...
	public function downloadJsonBackupFile(){
		$var = file_get_contents("php://input");
		$obj = json_decode($var, true);
	...
	public function saveServiceCredsFirstTime(){
		...
		foreach ($credObj as $key => $value) {
			...
			if($key == "facebook"){
				$scope = "user_hometown,user_location,publish_actions,read_stream,read_friendlists,email,user_likes,manage_notifications";
