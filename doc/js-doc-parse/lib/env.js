...
define([
	...
	'require',
	'exports'
], function (lang, Scope, File, Value, Module, console, args, require, exports) {
	/**
	...
	if (!args.config) {
		throw new Error('Missing required "config" command-line option.');
	}

	require([ 'dojo/node!path', args.config ], function (pathUtil, config) {
		exports.config = config;

		// change the dir to the dir of the config file so that filenames taken from the config will
		// resolve relative to the config file rather than the dir where the command was executed.
		process.chdir(pathUtil.dirname(args.config));
